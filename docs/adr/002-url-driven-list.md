...
- List state lives in the URL: `search`, `segment`, `page`, `page_size`, `sort`, `order`.
- Reset `page` to `1` when search, segment, sort, or page size change; clamp `page` when the result set shrinks.
- When `sort` / `order` are absent, resolve default **health (risk-first) then name** in parse/query — do not require those params on first land.
