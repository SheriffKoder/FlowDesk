...
Dumb dual-triangle sort toggle for table header cells. No router / URL knowledge.

Adapted from `app/development/url-kit/sorting` `SortButton` (presentation only).

## Behavior

- Shows stacked filled Lucide `Triangle` icons (`flex-col`); active direction uses `text-primary`.
